...
export function Viewer(data, parent, width, height, font) {
  ...
  for (i = 0; i < data.entities.length; i++) {
    ...
    if (obj) {
      // Bring duplicates to front
      if (entity.layer === "DUPLICATES") {
        obj.position.z += 0.5;
        obj.renderOrder = 999;
        if (obj.material) {
  ...
  var initialZoomTop = viewPort.top;
  ...
  controls.addEventListener("change", () => {
    ...
    if (this.onZoomChange) {
      var currentZoom = initialZoomTop / camera.top;
  ...
  this.resetView = function () {
    ...
    camera.position.y = viewPort.center.y;
    camera.position.z = 10;
    controls.target.x = viewPort.center.x;
    controls.target.y = viewPort.center.y;
    controls.target.z = 0;
  ...
  function drawLine(entity, data) {
    ...
    var material, lineType, vertex, startPoint, endPoint, bulge, i, line;
